...
def global_variables_view():
    try:
        ...
        print(get_variables_and_print(session, auth_token, projects_dict))
...
def get_variables_and_print(session: requests.Session, auth_token: str, projects_dict: dict) -> None:
    results = []
    for project_uuid, project_info in projects_dict.items():
        if project_uuid == '01ed1955-7613-8448-08af-00b15c0c4000':
            continue
        headers = {
            'Content-Type': Config.CONTENT_TYPE,
            'Authorization': f'Bearer {auth_token}',
        ...
        try:
            ...
            if 'defaultVariables' in variables:
                for var in variables['defaultVariables']:
                    project_name = project_info.get('name', 'Н/Д')
    ...
    return results
            # except HTTPError as e:
            #     print(f"Ошибка HTTP при запросе к проекту с UUID {project_uuid}: {e}")
            # except Exception as e:
            #     print(f"Ошибка при запросе к проекту с UUID {project_uuid}: {e}")
                #print(project_name)
                #project_description = project_info.get('description', 'Н/Д')
                #results.append(f"{var['value']['name']} - {var['eventType']} - {var['projectUuid']} - {var['uuid']} - {project_name} ")
                #return (f"{var['value']['name']} - {var['eventType']} - {var['projectUuid']} - {var['uuid']} - {project_name} - {project_description}")
                #print(f"{var['value']['name']} - {var['eventType']} - {var['projectUuid']} - {var['uuid']} - {project_name} - {project_description}")
        #return results
